...
YRot = [ cosd(YAngle)     0        sind(YAngle);
              0           1            0
        -sind(YAngle)     0        cosd(YAngle)]; %Matrice di rotazione attorno all’asse y per l’angolo YAngle. Questa rotazione modifica la freccia (sweep angle) e altre configurazioni legate alla geometria dell’ala.
    
XAngle = config.DihedralAngle(iCorpo);
XRot = [ 1         0              0      ;
         0    cosd(XAngle)  -sind(XAngle);
         0    sind(XAngle)   cosd(XAngle) ]; % Matrice di rotazione attorno all’asse x per l’angolo di diedro (DihedralAngle). Simula l’inclinazione verso l’alto o verso il basso delle estremità dell’ala.

WingExtremes.RootLE = [0 0 0]; % RootLE: BA alla radice dell' ala
